...
% cam1:
    [x1, y1] = paintcan_find(vidFrames1_4, filter1, 2, 'average');  
    ...
    xlim([0,size(vidFrames1_4, 4)+70])
    title(['Displacement of Paint Can, w/ Horizontal Disp + Rotation (cam1)'])
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')
...
% cam2:
    ...
    filter2(:,250:450) = 1; 
    [x2, y2] = paintcan_find(vidFrames2_4, filter2, 4, 'average');
    ...
    title(['Displacement of Paint Can, w/ Horizontal Disp + Rotation (cam2)'])
    xlim([0,size(vidFrames2_4, 4)+70])
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')
...
% cam3:
    ...
    filter3(145:280, 250:end) = 1;
    [x3, y3] = paintcan_find(vidFrames3_4, filter3, 8, 'average');
    ...
    title(['Displacement of Paint Can, w/ Horizontal Disp + Rotation (cam3)'])
    xlim([0,size(vidFrames3_4, 4)+70])   
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')


X = [x1; y1; x2(1:length(x1)); y2(1:length(x1)); x3(1:length(x1)); y3(1:length(x1))];
...
xlabel(['Variance'])
ylabel(['Energy'])
title(['Energy, Diagonal Variances, Case 4'])
xlim([0 7])
ylim([0 1])
