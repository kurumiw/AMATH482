...
% cam1:
    [x1, y1] = paintcan_find(vidFrames1_2, filter1, 2);
    ...
    xlim([0,size(vidFrames1_2, 4)+50])
    title(['Displacement of Paint Can, Noisy case (cam1)'])
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')
...
% cam2:
    ...
    filter2(:,250:450) = 1;
    [x2, y2] = paintcan_find(vidFrames2_2, filter2, 4);
    ...
    title(['Displacement of Paint Can, Noisy case (cam2)'])
    xlim([0,size(vidFrames2_2, 4)+50])
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')
...
% cam3:
    ...
    [x3, y3] = paintcan_find(vidFrames3_2, filter3, 6);
    ...
    title(['Displacement of Paint Can, Noisy case (cam3)'])
    xlim([0,size(vidFrames3_2, 4)+50])
    xlabel(['Frame'])
    ylabel(['Displacement'])
    legend('Z-Axis', 'X-Y')
...
xlabel(['Variance'])
ylabel(['Energy'])
title(['Energy, Diagonal Variances, Case 2'])
